...
export default function Page() {
  const { data, error, isLoading, mutate } = useSWR("/api/network/vpn", fetcher)
  const raw = Array.isArray(data) ? data : (data?.gateways || [])
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <PageHeader title="Customer Gateways" />
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-white/10 text-white/60">
              ...
              <th className="px-4 py-3 text-left">Gateway IP</th>
