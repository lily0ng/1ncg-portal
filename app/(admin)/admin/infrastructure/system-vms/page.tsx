...
export default function Page() {
  const { data, error, isLoading, mutate } = useSWR("/api/infrastructure/system-vms", fetcher)
  const raw = Array.isArray(data) ? data : (data?.systemvm || [])
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <PageHeader title="System VMs" />
