...
import { Plus, Pencil, Trash2, AlertTriangle, RefreshCw, X, Check, HardDrive } from 'lucide-react'
...
export default function DiskOfferingsPage() {
  const { data, isLoading, error, mutate } = useSWR('/api/service-offerings/disk', fetcher)
  const [showModal, setShowModal] = useState(false)
  const [editOffering, setEditOffering] = useState<DiskOffering | null>(null)
  const [form, setForm] = useState<FormData>(defaultForm)
  const [saving, setSaving] = useState(false)
  const [deleteId, setDeleteId] = useState<string | null>(null)
  const [deleting, setDeleting] = useState(false)

  const offerings: DiskOffering[] = data?.diskofferings || data?.offerings || []
  ...
  function openEdit(o: DiskOffering) {
    setEditOffering(o)
    setForm({
      name: o.name || '',
      displaytext: o.displaytext || '',
      disksize: String(o.disksize || 0),
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      const url = editOffering
        ? `/api/service-offerings/disk/${editOffering.id}`
        : '/api/service-offerings/disk'
      const res = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...form, disksize: parseInt(form.disksize) }),
      })
      if (!res.ok) throw new Error('Failed to save offering')
      toast.success(editOffering ? 'Disk offering updated' : 'Disk offering created')
      setShowModal(false)
      mutate()
    } catch (e: any) {
  ...
  async function handleDelete(id: string) {
    setDeleting(true)
    try {
      const res = await fetch(`/api/service-offerings/disk/${id}`, { method: 'DELETE' })
      if (!res.ok) throw new Error('Failed to delete')
      toast.success('Disk offering deleted')
      setDeleteId(null)
      mutate()
    } catch (e: any) {
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      <motion.div variants={itemVariants}>
        <PageHeader
          ...
          description="Manage disk service offerings for volumes"
          action={
            <button
              onClick={openCreate}
            ...
            >
              <Plus className="w-4 h-4" /> Create Disk Offering
            </button>
          }
        />
      </motion.div>
      ...
      {error && (
        <motion.div variants={itemVariants} className="flex items-center gap-3 bg-red-500/10 border border-red-500/20 rounded-xl p-4 text-red-400">
          ...
          <p className="text-sm">Failed to load disk offerings.</p>
            ...
            <table className="w-full text-sm min-w-[900px]">
              <thead>
                <tr className="border-b border-white/10 text-white/60 text-xs uppercase tracking-wider">
                  {['Name', 'Size (GB)', 'Storage Type', 'Custom Size', 'Tags', 'Public', 'Created', 'Actions'].map((h) => (
              ...
              <tbody className="divide-y divide-white/5">
                {offerings.length === 0 ? (
                  <tr><td colSpan={8} className="px-4 py-10 text-center text-white/40">No disk offerings found</td></tr>
                ) : offerings.map((o) => (
                  <tr key={o.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-4 py-3 text-white/80">
                      {o.iscustomized ? <span className="text-yellow-400 text-xs font-medium">Custom</span> : `${o.disksize ?? 0} GB`}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      {o.iscustomized ? <Check className="w-4 h-4 text-green-400" /> : <X className="w-4 h-4 text-white/30" />}
                    </td>
                    <td className="px-4 py-3 text-white/60 text-xs">{o.tags || '-'}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => openEdit(o)} className="p-1.5 rounded-lg bg-white/5 hover:bg-indigo-500/20 text-white/60 hover:text-indigo-400 transition-colors">
                          <Pencil className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => setDeleteId(o.id)} className="p-1.5 rounded-lg bg-white/5 hover:bg-red-500/20 text-white/60 hover:text-red-400 transition-colors">
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-semibold text-white">
                  {editOffering ? 'Edit Disk Offering' : 'Create Disk Offering'}
                </h2>
              </div>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-white/60 mb-1.5">Size (GB, 0 = Custom)</label>
                  <input
                    type="number"
                    min="0"
                    value={form.disksize}
                    onChange={(e) => setForm((f) => ({ ...f, disksize: e.target.value }))}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-base font-semibold text-white">Delete Disk Offering</h2>
